...
const MonthlySummary: React.FC<MonthlySummaryProps> = ({ notes }) => {
  ...
  const summaryText = filteredNotes.map(note => 
    `- ${new Date(note.date).toLocaleDateString()}: Proyecto "${note.project}" (${calculateTotalHours(note.startTime, note.endTime).toFixed(2)} horas)`
  ).join('\n');
            ...
            <table className="min-w-full bg-white border border-slate-200">
              ...
              <tbody className="divide-y divide-slate-200">
                {filteredNotes.length > 0 ? filteredNotes.map(note => (
                  ...
                  <tr>
                    <td colSpan={4} className="text-center py-8 text-slate-500">No hay datos para el período seleccionado.</td>
                  </tr>
                )}
              </tbody>
              <tfoot className="bg-slate-100">
                  <tr>
                      <td colSpan={3} className="py-3 px-4 text-right font-bold text-slate-700 uppercase">Total de Horas</td>
