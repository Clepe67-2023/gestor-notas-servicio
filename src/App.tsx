...
const Home = ({ notes, clients, projects, consultants }: { notes: ServiceNoteType[], clients: Client[], projects: Project[], consultants: Consultant[] }) => {
    
    const findProject = (id: string) => projects.find(p => p.id === id);
    const findClient = (id: string) => clients.find(c => c.id === id);
    const findConsultant = (id: string) => consultants.find(c => c.id === id);

    const getNoteDetails = (note: ServiceNoteType) => {
        const project = findProject(note.projectId);
        const client = project ? findClient(project.clientId) : undefined;
        const consultant = findConsultant(note.consultantId);
                            ...
                            return (
                                <Link to={`/note/${note.id}`} key={note.id} className="block p-4 bg-slate-50 border border-slate-200 rounded-lg hover:shadow-lg hover:border-blue-500 transition duration-300">
                                    <div className="flex justify-between items-center">
                                        <div>
                                            <p className="font-bold text-lg text-slate-800">{client?.name || 'Cliente no encontrado'}</p>
                                            <p className="text-sm text-slate-600">Proyecto: {project?.name || 'Proyecto no encontrado'}</p>
                                            <p className="text-xs text-slate-500 mt-1">Asesor: {consultant?.name || 'Asesor no encontrado'}</p>
                                        </div>
...
const ServiceNoteWrapper = ({ notes, setNotes, clients, projects, consultants }: { notes: ServiceNoteType[], setNotes: (notes: ServiceNoteType[]) => void, clients: Client[], projects: Project[], consultants: Consultant[] }) => {
    ...
    return (
        <HashRouter>
            <div className="min-h-screen bg-slate-100 font-sans">
                    ...
                    <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="flex items-center justify-between h-20">
                            ...
                            <div className="flex items-center space-x-6">
                                ...
                                <Link to="/clients" className="text-slate-600 hover:text-blue-600 font-medium transition">Clientes</Link>
                                <Link to="/projects" className="text-slate-600 hover:text-blue-600 font-medium transition">Proyectos</Link>
                                <Link to="/consultants" className="text-slate-600 hover:text-blue-600 font-medium transition">Asesores</Link>
                                <Link to="/summary" className="text-slate-600 hover:text-blue-600 font-medium transition">Resumen</Link>
                            </div>
                        </div>
                    </nav>
                </header>
                <main>
                    <Routes>
                        <Route path="/" element={<Home notes={notes} clients={clients} projects={projects} consultants={consultants} />} />
                        ...
                        <Route path="/clients" element={<ClientsManager clients={clients} setClients={setClients} projects={projects} />} />
                        <Route path="/projects" element={<ProjectsManager projects={projects} setProjects={setProjects} clients={clients} notes={notes} />} />
                        <Route path="/consultants" element={<ConsultantsManager consultants={consultants} setConsultants={setConsultants} notes={notes} />} />
                    </Routes>
                </main>
            </div>
        </HashRouter>
